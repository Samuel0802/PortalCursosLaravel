...
<div>
    ...
    <form action="{{ route('users.store') }}" method="post">
       ...
       <input type="text" name="name" id="name" placeholder="Digite o Nome Completo" value="{{ old('name') }}"/><br><br>
         ...
         <label>Login:</label>
       <input type="text" name="login" id="login" placeholder="Digite o login" value="{{ old('login') }}"/><br><br>

         <label>Email:</label>
       <input type="text" name="email" id="email" placeholder="Digite o Email" value="{{ old('email') }}"/><br><br>
       ...
       <label>Permissão:</label>
        @forelse ($permissoes as $permissao)
           ...
           @if($permissao != 'Super Admin' || (auth()->check() && auth()->user()->hasRole('Super Admin')))
             <input type="checkbox" name="permissoes[]" id="permissao_{{Str::slug($permissao)}}" value="{{ $permissao }}"
              {{  collect(old('permissoes'))->contains($permissao) ? 'checked' : '' }}>
